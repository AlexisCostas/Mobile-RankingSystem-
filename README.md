...
- `MobileRankingSystem/` – .NET MAUI single-project structure containing:
  - `App.cs` – application bootstrap that wires up resources and the root navigation page.
  - `MainPage.cs` – C#-authored page that lists calculated team rankings.
  ...
  - `Platforms/Android` – Android entry point and manifest required by the MAUI tooling.
  - `Resources/` – shared fonts, splash screen, and app icon assets used by the application.
...
1. Install the [.NET 8 SDK](https://dotnet.microsoft.com/en-us/download) and the MAUI workload for Android by following the [official setup guide](https://learn.microsoft.com/dotnet/maui/get-started/installation).
...
3. In Visual Studio, ensure that the **MobileRankingSystem** project is set as the startup project and choose an Android emulator or device from the run target dropdown before pressing **Run**. The included Android head provides the necessary entry point for deployment.
4. Deploy to an Android emulator or device. The project is scoped to Android, so no additional platform tooling is required.
